<template>
    <div>
        <h2 class="title">Food and Bar Editiern</h2>
...
export default {
    ...
    methods: {
        submit() {
            ...
            if (!this.$v.$invalid) {
                ...
                auth.put(`bars/${this.$route.params.id}`, convertToFormData(formData))
                    .then(response => {
    ...
    mounted() {
        auth.get(`bars/${this.$route.params.id}`)
            .then(response => {
